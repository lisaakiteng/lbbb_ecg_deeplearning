...
from models_resnet_fusion import fusion_resnet_medium
...
OUTPUT_DIR = "outputs_resnet_fusion"
...
EXPERIMENT_NAME = "exp3_resnet_medium_k7_fusion"
...
# =========================================
# TABULAR FEATURES — SAME AS PREVIOUS FUSION
# =========================================
...
# =========================================
# ECG LEADS — SAME AS PREVIOUS FUSION
# =========================================
...
models = [
    ("fusion_resnet_medium_k7", lambda: fusion_resnet_medium(n_tab=n_tab, k=7)),
]
...
for lr in lrs:
    for name, model_fn in models:
        ...
        df_val_mean.to_csv(
            os.path.join(PRED_DIR, f"{run_name}_val_patient_mean.csv"),
            index=False,
        )

        df_val_max.to_csv(
            os.path.join(PRED_DIR, f"{run_name}_val_patient_max.csv"),
            index=False,
        )

        df_test_mean.to_csv(
            os.path.join(PRED_DIR, f"{run_name}_test_patient_mean.csv"),
            index=False,
        )

        df_test_max.to_csv(
            os.path.join(PRED_DIR, f"{run_name}_test_patient_max.csv"),
            index=False,
        )
