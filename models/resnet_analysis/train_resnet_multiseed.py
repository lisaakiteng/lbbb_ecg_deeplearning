...
OUTPUT_DIR = "outputs_resnet_multiseed"
...
EXPERIMENT_NAME = "exp2_resnet_medium_k7_multiseed"
...
if "y_old_ever_crt" in train_df.columns:
    ...
    for split_name, df in [("train", train_df), ("val", val_df), ("test", test_df)]:
        ...
        print("new label ECG positives:", int(df["y"].sum()))
...
test_ds = make_ds(test_packed, y_test, batch_size=batch_size, training=False)
...
def save_roc_curve(dfp, auc_value, run_name, split):
    ...
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.title(f"{split.upper()} Patient-level ROC - {run_name}")
    plt.legend()
...
models = [
    # ("resnet1d_light", resnet1d_light),
    ("resnet1d_medium_k7", resnet1d_medium_k7),
    # ("resnet1d_medium_k15", resnet1d_medium_k15),
    # ("resnet1d_deep_k7", resnet1d_deep_k7),
]
...
for lr in lrs:
    for seed in seeds:
        ...
        run_name = f"{EXPERIMENT_NAME}_{name}_lr{lr:g}_seed{seed}"
...
summary_df=(results_df.groupby(["experiment","model","lr"])
.agg(n_runs=("seed","count"),
val_auc_mean_mean=("val_auc_mean","mean"),
val_auc_mean_std=("val_auc_mean","std"),
test_auc_mean_mean=("test_auc_mean","mean"),
test_auc_mean_std=("test_auc_mean","std")).reset_index()
summary_df.to_csv(os.path.join(RESULT_DIR,f"{EXPERIMENT_NAME}_mean_std_summary.csv"),index=False)
