...
from models_cnn_lstm_fusion import fusion_cnn_lstm
...
OUTPUT_DIR = "/data/leuven/376/vsc37666/lbbb_outputs/outputs_cnn_lstm_fusion_multiseed"
...
EXPERIMENT_NAME = "cnn_lstm_fusion_multiseed"
...
models = [
    ("fusion_cnn_lstm", lambda: fusion_cnn_lstm(n_tab=n_tab))
]
...
for seed in seeds:
    ...
    for lr in lrs:
        for name, model_fn in models:
            ...
            run_name = f"{EXPERIMENT_NAME}_{name}_seed{seed}_lr{lr:g}"
...
summary_path = os.path.join(RESULT_DIR, f"{EXPERIMENT_NAME}_summary_by_seed.csv")
