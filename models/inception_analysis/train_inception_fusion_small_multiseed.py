# train_inception_fusion_small_multiseed.py
# 10-seed rerun of best 3-block Inception Fusion kernel model: (5, 11, 21)
...
from models_inception_fusion_kernels import fusion_inception_light_small


DATA_DIR = "/data/leuven/376/vsc37666/lbbb_data/"
OUTPUT_DIR = "/data/leuven/376/vsc37666/lbbb_outputs/outputs_inception_fusion_small_k5_11_21_multiseed"
...
for d in [MODEL_DIR, PLOT_DIR, PRED_DIR, RESULT_DIR]:
    os.makedirs(d, exist_ok=True)


EXPERIMENT_NAME = "exp1_inception_fusion_small_k5_11_21_multiseed"

model_name = "fusion_inception_light_small_k5_11_21"
...
for lr in lrs:
    for seed in seeds:
        ...
        model = fusion_inception_light_small(n_tab=n_tab)
...
summary_df = (
    results_df
    .groupby(["experiment", "model", "lr", "kernel_sizes", "blocks"])
    .agg(
        n_runs=("seed", "count"),
...
print(f"Kernels: {best_individual['kernel_sizes']}")
print(f"Blocks: {best_individual['blocks']}")
...
print(f"Kernels: {best_summary['kernel_sizes']}")
print(f"Blocks: {int(best_summary['blocks'])}")
